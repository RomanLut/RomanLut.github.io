
# Template rule for converting wordpress article

[location]=D:\Github\PersonalPage\trunk\temp\article.html

[target]=\trunk\public\filesystem\Electronics\2014-04_Interactive_TV_backlight\article.md

Read the article [location] fully.

Convert article to [target]

Do not translate.

place images into /images subfolder. use relative path like images/01.jpg in .md file.

Make sure links are converted as links and code is displayed as code blocks.
Convert plain text links to markdown links.
Convert Youtube iframe embeddings to links[]().

Do not use script to convert article. Read and undestand it.

Try to preserve original formating.
Preserve indentation in code blocks.
Apply nice styling  - heading, indentation in .md file. 

Do not install aditional nodejs/python packages.

Double check result.



# Template rule for translating wordpress article

Translate article to english to article_en.md.
Reread article from disk, it was changed.
Переведи дословно, но используй fluent english.
Preserve formatting.



